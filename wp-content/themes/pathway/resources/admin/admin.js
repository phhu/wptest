...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".pathway-admin-big-notice--container");
  ...
  function disableNotice() {
    ...
    $notice_container.closest(".pathway-admin-big-notice").find("button.notice-dismiss").click();
  }

  function toggleProcessing(value) {
    $(window).on("beforeunload.pathway-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".pathway-admin-big-notice").addClass("processing");
      $(".pathway-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".pathway-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.pathway-admin-big-notice");
    });
  }
  ...
  $notice_container.closest('.pathway-admin-big-notice').on("click", ".pathway-notice-dont-show-container", disableNotice);
  ...
  window.pathway_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
